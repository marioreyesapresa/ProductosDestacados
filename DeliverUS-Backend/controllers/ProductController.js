...
exports.promote = async function (req, res) {
  ...
  try {
    ...
    await t.commit()
    res.status(200).send('Product promoted succesfully')
  } catch (err) {
    await t.rollback()
    res.status(500).send(err)
  }
...
exports.highlight = async function (req, res) {
  const t = await models.sequelize.transaction()
  try {
    const highlightProductCount = await Product.count({ where: { highlight: true } })
    if (highlightProductCount) {
      const firstHighlightedProduct = await Product.findOne({ // cual fue el primer producto destacado
        where: { highlight: true },
        order: [['createdAt', 'DESC']]
      })
      await Product.update(
        { highlight: false },
        { where: { id: firstHighlightedProduct.id } },
        { transaction: t }
      )
    }
    await Product.update(
    ...
    await t.commit()
    res.status(200).send('Product highlight succesfully')
  } catch (err) {
    await t.rollback()
    res.status(500).send(err)
  }
